    ...
    let o,
      ...
      d = [].slice.call(arguments, 1); return r.each(function () {
        let g,
          ...
          b = e(),
          y = this,
          k = h.data(v); g = { initialize() { m.count ? g.store(m.key.count, m.count) : m.id ? g.add.id(m.id) : m.increment && l !== 'increment' && g.increment(), g.add.display(h), g.instantiate(); },
            ...
            get: { count() { return +g.retrieve(m.key.count) || 0; }, idCount(e) { return e = e || g.get.ids(), e.length; }, ids(e) { let t = []; return e = e || g.retrieve(m.key.ids), typeof e === 'string' && (t = e.split(m.delimiter)), g.verbose('Found visited ID list', t), t; }, storageOptions(e) { const t = {}; return m.expires && (t.expires = m.expires), m.domain && (t.domain = m.domain), m.path && (t.path = m.path), t; } },
            has: { visited(t, i) { let o = !1; return i = i || g.get.ids(), t !== n && i && e.each(i, (e, i) => { i == t && (o = !0); }), o; } },
            set: { count(e) { g.store(m.key.count, e); }, ids(e) { g.store(m.key.ids, e); } },
            reset() { g.store(m.key.count, 0), g.store(m.key.ids, null); },
            add: { id(e) {
              let t = g.retrieve(m.key.ids),
              ...
              display(t) { const i = e(t); i.length > 0 && !e.isWindow(i[0]) && (g.debug('Updating visit count for element', i), b = b.length > 0 ? b.add(i) : i); } },
            remove: { id(t) {
              let i = g.get.ids(),
                o = []; t !== n && i !== n && (g.debug('Removing visit to unique content', t, i), e.each(i, (e, i) => { i !== t && o.push(i); }), o = o.join(m.delimiter), g.store(m.key.ids, o)), g.set.count(g.get.idCount());
            } },
            ...
            performance: { log(e) {
              let t,
                i,
                n; m.performance && (t = (new Date()).getTime(), n = a || t, i = t - n, a = t, c.push({ Name: e[0], Arguments: [].slice.call(e, 1) || '', Element: y, 'Execution Time': i })), clearTimeout(g.performance.timer), g.performance.timer = setTimeout(g.performance.display, 500);
            },
              display() {
                let t = `${m.name}:`,
                  i = 0; a = !1, clearTimeout(g.performance.timer), e.each(c, (e, t) => { i += t['Execution Time']; }), t += ` ${i}ms`, s && (t += ` '${s}'`), r.length > 1 && (t += ` (${r.length})`), (console.group !== n || console.table !== n) && c.length > 0 && (console.groupCollapsed(t), console.table ? console.table(c) : e.each(c, (e, t) => { console.log(`${t.Name}: ${t['Execution Time']}ms`); }), console.groupEnd()), c = [];
              } },
            invoke(t, i, r) {
              let s,
                a,
                c,
                u = k; return i = i || d, r = y || r, typeof t === 'string' && u !== n && (t = t.split(/[\. ]/), s = t.length - 1, e.each(t, (i, o) => { const r = i != s ? o + t[i + 1].charAt(0).toUpperCase() + t[i + 1].slice(1) : t; if (e.isPlainObject(u[r]) && i != s)u = u[r]; else { if (u[r] !== n) return a = u[r], !1; if (!e.isPlainObject(u[o]) || i == s) return u[o] !== n ? (a = u[o], !1) : !1; u = u[o]; } })), e.isFunction(a) ? c = a.apply(r, i) : a !== n && (c = a), e.isArray(o) ? o.push(c) : o !== n ? o = [o, c] : c !== n && (o = c), a;

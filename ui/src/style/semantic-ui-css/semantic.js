      ...
      module = {
        ...
        remove: {
          rule(field, rule) {
            ...
            $.each(validation[field].rules, (index, rule) => {
...
;
      ...
      module = {
        ...
        event: {
          ...
          test: {
            ...
            touch(event) {
              module.determine.eventOnElement(event, () => {
        ...
        get: {
          ...
          userValues() {
            ...
            values = $.isArray(values)
              ...
              : [values]
            ;
            return $.grep(values, (value) => (module.get.item(value) === false));
          },
          uniqueArray(array) {
            return $.grep(array, (value, index) => $.inArray(value, array) === index);
          },
          caretPosition() {
            let
              input = $search.get(0),
          ...
          choiceText($choice, preserveHTML) {
          ...
          selectValues() {
            ...
            if (settings.sortSelect) {
              select.values.sort((a, b) => (a.name > b.name)
                  ? 1
                  : -1);
            ...
            if (shouldSearch) {
              $item
                .each(function () {
                  ...
                  } else if (String(optionValue) == String(value) || optionText == value) {
        ...
        scrollPage(direction, $selectedItem) {
          let
        ...
        set: {
          ...
          scrollPosition($item, forceScroll) {
            let
              edgeTolerance = 5,
              $menu,
              hasActive,
        ...
        add: {
          label(value, text, shouldAnimate) {
            let
          ...
          value(addedValue, addedText, $selectedItem) {
            let
              currentValue = module.get.values(),
              newValue
              ;
        ...
        remove: {
          ...
          value(removedValue, removedText, $removedItem) {
            let
              values = module.get.values(),
              newValue
              ;
          ...
          arrayValue(removedValue, values) {
            if (!$.isArray(values)) {
              values = [values];
            }
            values = $.grep(values, (value) => (removedValue != value));
            module.verbose('Removed value from delimited string', removedValue, values);
            return values;
          },
          label(value, shouldAnimate) {
            let
              $labels = $module.find(selector.label),
        ...
        has: {
          ...
          valueIgnoringCase(value) {
            ...
            $.each(values, (index, existingValue) => {
        ...
        is: {
          ...
          bubbledLabelClick(event) {
            return $(event.target).is('select, input') && $module.closest('label').length > 0;
          },
          bubbledIconClick(event) {
          ...
          inObject(needle, object) {
            let
              found = false
            ;
            $.each(object, (index, property) => {
              if (property == needle) {
                found = true;
                return true;
              }
  ...
  /* Callbacks */
    ...
    onLabelCreate(value, text) { return $(this); },
    onLabelRemove(value) { return true; },
    onNoResults(searchTerm) { return true; },
    onShow() {},
    onHide() {},
  ...
  $.fn.tab = function (parameters) {
    ...
    .each(function () {
      ...
      module = {
        ...
        update: {
          content(tabPath, html, evaluateScripts) {
            let
              $tab = module.get.tabElement(tabPath),
              tab = $tab[0]
              ;
        ...
        utilities: {
          filterArray(keepArray, removeArray) {
            return $.grep(keepArray, (keepValue) => ($.inArray(keepValue, removeArray) == -1));
          },
          last(array) {
...
// shortcut for tabbed content with no defined navigation
  ...
  $.fn.tab.settings = {
    ...
    onFirstLoad(tabPath, parameterArray, historyEvent) {}, // called first time loaded
    onLoad(tabPath, parameterArray, historyEvent) {}, // called on every load
    onVisible(tabPath, parameterArray, historyEvent) {}, // called every time tab visible
    onRequest(tabPath, parameterArray, historyEvent) {}, // called ever time a tab beings loading remote content

    templates: {
      determineTitle(tabArray) {}, // returns page title for path
    },
      ...
      module = {
        ...
        passed(amount, newCallback) {
          ...
          if (amount && newCallback) {
            ...
            $.each(settings.onPassed, (amount, callback) => {
